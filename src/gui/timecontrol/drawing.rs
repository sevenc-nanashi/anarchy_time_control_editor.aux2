...
impl TimeControlEditorApp {
    ...
    pub fn draw_timecontrol_control_lines(
        ...
        for segment_index in 0..timecontrol.segments.len() {
            ...
            let Some(bezier) = timecontrol.segment_bezier(segment_index) else {
                unreachable!("ベジェ区間にはベジェハンドルが存在する");
            };
            let out_handle = viewport.graph_to_screen(bezier.start_handle);
            let in_handle = viewport.graph_to_screen(bezier.end_handle);
